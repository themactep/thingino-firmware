...
# I2C1 GPIO-bitbang board files are per-board (not per-SoC-family) -- Pike
# is currently the only board choice under soc-t23/chip-t23/isvp, and these
# scripts no-op if the target file doesn't exist (other SoC families/boards).
THINGINO_I2C1_GPIO_BOARD_H = $(LINUX_DIR)/arch/mips/xburst/soc-$(SOC_FAMILY)/chip-$(SOC_FAMILY)/isvp/Pike/board.h
THINGINO_I2C1_GPIO_CUSTOMIZED_C = $(LINUX_DIR)/arch/mips/xburst/soc-$(SOC_FAMILY)/chip-$(SOC_FAMILY)/isvp/Pike/gpio_customized.c
...
define THINGINO_KOPT_PREPARE_KERNEL
	...
	sh $(BR2_EXTERNAL_THINGINO_PATH)/scripts/patch_kernel_i2c1_gpio_pins.sh \
		$(BR2_CONFIG) \
		$(THINGINO_I2C1_GPIO_BOARD_H)
	sh $(BR2_EXTERNAL_THINGINO_PATH)/scripts/patch_kernel_i2c1_gpio_pullups.sh \
		$(BR2_CONFIG) \
		$(THINGINO_I2C1_GPIO_CUSTOMIZED_C)
