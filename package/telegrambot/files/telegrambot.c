...
static int http_get(const char *url, long timeout_s, Memory *out) {
    ...
    if (res != CURLE_OK || code != 200) {
        syslog(LOG_WARNING, "HTTP GET failed: %s (code=%ld)", curl_easy_strerror(res), code);
...
static int http_post_json(const char *url, const char *json, long timeout_s, Memory *out) {
    ...
    if (res != CURLE_OK || code != 200) {
        syslog(LOG_WARNING, "HTTP POST failed: %s (code=%ld)", curl_easy_strerror(res), code);
        free(out->data); out->data = NULL; out->size = 0;
        return -1;
    }
    return 0;
...
static void json_escape(const char *src, char *dst, size_t dstsz) {
    size_t j = 0; if (!dstsz) return; if (!src) { dst[0] = '\0'; return; }
...
static long read_long(JsonValue *obj, const char *key, long def) {
    ...
    if (it && it->type == JSON_NUMBER) return (long)it->value.number;
    return def;
}

static int read_bool(JsonValue *obj, const char *key, int def) {
    JsonValue *it = get_nested_item(obj, key);
    if (it && it->type == JSON_BOOL) return it->value.boolean;
    return def;
}
...
static int user_allowed(const Config *cfg, const char *username) {
    if (cfg->allowed_users_count == 0) return 1; // no username filter
    if (!username || !*username) return 0;
    for (int i = 0; i < cfg->allowed_users_count; ++i) {
        if (strcmp(cfg->allowed_users[i], username) == 0) return 1;
    }
    return 0;
}
...
static int find_command(const Config *cfg, const char *text) {
    ...
    for (int i = 0; i < cfg->cmd_count; ++i) {
        ...
        if (!h || !*h) continue;
        if (strcmp(text, h) == 0) return i;                 // exact match
        if (text[0] == '/' && strcmp(text + 1, h) == 0) return i; // allow "/" prefix
    }
    return -1;
}
...
static int run_command(const Config *cfg, int idx, long chat_id) {
    if (idx < 0 || idx >= cfg->cmd_count) return -1;
    const char *cmd = cfg->commands[idx].exec;
    if (!cmd || !*cmd) return -1;

    FILE *p = popen(cmd, "r");
    if (!p) return -1;
    char buf[896]; size_t n = fread(buf, 1, sizeof(buf)-1, p); buf[n] = '\0';
    int rc = pclose(p);
    (void)rc;
    if (n == 0) {
...
static int id_allowed(Config *cfg, long chat_id) {
    if (cfg->allowed_count == 0) return 1; // no filter
    for (int i = 0; i < cfg->allowed_count; ++i) if (cfg->allowed_ids[i] == chat_id) return 1;
    return 0;
}
...
static int load_config_file(const char *path, Config *cfg) {
    ...
    snprintf(cfg->state_file, sizeof(cfg->state_file), "%s", "/var/run/telegrambot.state");
    ...
    if (arr && arr->type == JSON_ARRAY) {
        ...
        for (int i = 0; i < n && cfg->allowed_count < (int)(sizeof(cfg->allowed_ids)/sizeof(cfg->allowed_ids[0])); ++i) {
            ...
            if (el && el->type == JSON_NUMBER) cfg->allowed_ids[cfg->allowed_count++] = (long)el->value.number;
        }
    }
    ...
    if (cmds && cmds->type == JSON_ARRAY) {
        ...
        for (int i = 0; i < n && cfg->cmd_count < (int)(sizeof(cfg->commands)/sizeof(cfg->commands[0])); ++i) {
            ...
            if (!h || h->type != JSON_STRING || !e || e->type != JSON_STRING) continue;
            snprintf(cfg->commands[cfg->cmd_count].handle, sizeof(cfg->commands[cfg->cmd_count].handle), "%s", h->value.string);
            snprintf(cfg->commands[cfg->cmd_count].description, sizeof(cfg->commands[cfg->cmd_count].description), "%s", (d && d->type == JSON_STRING)? d->value.string : "");
...
static long load_offset(const char *path) {
    FILE *f = fopen(path, "r");
    if (!f) return 0;
    long off = 0; fscanf(f, "%ld", &off); fclose(f); return off;
}
...
static int reply_text(const Config *cfg, long chat_id, const char *text) {
    ...
    Memory m; if (http_post_json(url, body, 20, &m) != 0) return -1;
    free(m.data);
    return 0;
}

static void process_update(const Config *cfg, JsonValue *upd) {
    if (!upd || upd->type != JSON_OBJECT) return;
    ...
    if (!chat || chat->type != JSON_OBJECT || !text || text->type != JSON_STRING) return;

    JsonValue *cidv = get_object_item(chat, "id");
    if (!cidv || cidv->type != JSON_NUMBER) return;
    ...
    if (from && from->type == JSON_OBJECT) {
        ...
        if (uname && uname->type == JSON_STRING) username = uname->value.string;
    }
    ...
    if (username && *username) syslog(LOG_INFO, "Message from %ld (@%s): %s", chat_id, username, t); else syslog(LOG_INFO, "Message from %ld: %s", chat_id, t);

    // Configured commands take precedence
    int ci = find_command(cfg, t);
    if (ci >= 0) { run_command(cfg, ci, chat_id); return; }

    // Built-ins
    if (strcmp(t, "/ping") == 0) {
        reply_text(cfg, chat_id, "pong");
    } else if (strcmp(t, "/time") == 0) {
        char buf[64]; time_t now = time(NULL); struct tm tm; localtime_r(&now, &tm); strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S", &tm);
        reply_text(cfg, chat_id, buf);
    } else if (strcmp(t, "/help") == 0) {
        char buf[512]; size_t off = 0; off += snprintf(buf+off, sizeof(buf)-off, "Commands:\n");
...
static int poll_once(const Config *cfg, long *offset_io) {
    char qs[64] = {0};
    if (*offset_io > 0) snprintf(qs, sizeof(qs), "offset=%ld", *offset_io + 1);
    char url[512];
    char qs2[80];
    snprintf(qs2, sizeof(qs2), "%s%stimeout=%d", qs, (qs[0]?"&":""), cfg->polling_timeout);
    make_url(url, sizeof(url), cfg, "getUpdates", qs2);

    Memory m; if (http_get(url, cfg->polling_timeout + 10, &m) != 0) return -1;

    JsonValue *root = parse_json_string(m.data);
    free(m.data);
    if (!root || root->type != JSON_OBJECT) { free_json_value(root); return -1; }

    JsonValue *ok = get_object_item(root, "ok");
    if (!ok || ok->type != JSON_BOOL || !ok->value.boolean) { free_json_value(root); return -1; }

    JsonValue *res = get_object_item(root, "result");
    if (!res || res->type != JSON_ARRAY) { free_json_value(root); return 0; }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (uid > *offset_io) *offset_io = uid;
...
static void daemonize_self(void) {
    ...
    if (fd0 >= 0) { dup2(fd0, 0); dup2(fd0, 1); dup2(fd0, 2); if (fd0 > 2) close(fd0);}
}
...
int main(int argc, char **argv) {
    ...
    if (cfg.daemonize) daemonize_self();
    ...
    while (g_running) {
        ...
        if (rc >= 0) {
            if (offset > 0) save_offset(cfg.state_file, offset);
        } else {
