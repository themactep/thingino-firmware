#!/usr/bin/env python3
"""Inject a T40 pre-verification init-table bypass into a firmware image.

Same mechanism as the T32 bypass: writes an INGE init-table entry that
clears the secure-boot enable flag before the bootrom performs signature
verification.  T40 uses a different secureboot register address.
"""
...
def default_output_path(input_path: Path) -> Path:
    return input_path.with_name(f"{input_path.stem}-t40-init-bypass{input_path.suffix}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inject a T40 pre-verification init-table bypass into a firmware image."
    )
    parser.add_argument("input", type=Path, help="Path to input firmware image")
    parser.add_argument(
        "-o", "--output", type=Path, help="Path to patched output image (default: add -t40-init-bypass)"
    )
    parser.add_argument(
        "--boot-offset",
        type=parse_int,
    ...
    parser.add_argument(
        ...
        type=parse_int,
        default=0x800000B0,
        help="32-bit address to write from the injected init table (default: 0x800000b0)",
    )
    parser.add_argument(
        "--write-value",
        type=parse_int,
        default=0x00004000,
        help="32-bit value to write to --target-addr (default: 0x00004000)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
