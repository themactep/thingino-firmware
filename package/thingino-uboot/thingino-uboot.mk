...
define THINGINO_PATCH_DEV_ENV
	@if [ -f $(@D)/include/configs/isvp_common.h ] && [ -f $(THINGINO_BINARIES_DIR)/uImage ] && [ -f $(THINGINO_BINARIES_DIR)/rootfs.squashfs ]; then \
		...
		KERNEL_OFFSET=$$(( $(UB_ENV_OFFSET) + $(UB_ENV_PARTITION_SIZE) )); \
		...
		MTDPARTS="$(THINGINO_UBOOT_FLASH_CONTROLLER):$(U_BOOT_SIZE_KB)k(boot),$(UB_ENV_SIZE_KB)k(env),$${KERNEL_SIZE_KB}k(kernel),$${ROOTFS_SIZE_KB}k(rootfs),$${DATA_SIZE_KB}k(data),$${UPGRADE_SIZE_KB}k@$${KERNEL_OFFSET_HEX}(upgrade),$${FLASH_SIZE_KB}k@0(all)"; \
