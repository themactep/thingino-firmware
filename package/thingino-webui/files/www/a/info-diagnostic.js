(function () {
  ...
  const renderResult = (payload) => {
    ...
    let rawOutput = "";
    const encoded =
      typeof payload.output_b64 === "string" ? payload.output_b64.trim() : "";
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        const message =
          data?.error?.message ||
          `Failed to generate diagnostics (HTTP ${response.status})`;
  ...
  uploadBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        const message =
          data?.error?.message || `Upload failed (HTTP ${response.status})`;
        throw new Error(message);
      }
      ...
      uploadBtn.innerHTML =
        '<i class="bi bi-cloud-upload me-1"></i>Upload to Server';
    }
  });
