(function () {
  ...
  async function loadConfig(options = {}) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Failed to load remote logging settings");
      ...
      showAlert(
        "danger",
        err.message || "Unable to load remote logging settings.",
      );
    } finally {
      if (!preserveBusy) {
        toggleBusy(false);
  ...
  async function saveConfig(payload) {
    ...
    try {
      ...
      showAlert(
        "danger",
        err.message || "Failed to save remote logging settings.",
      );
    } finally {
      toggleBusy(false);
    }
