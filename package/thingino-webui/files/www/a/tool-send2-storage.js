(function () {
  ...
  async function loadConfig() {
    await send2Load("Storage", async (data) => {
      ...
      $("#storage_device_path").value =
        storage.device_path || "%hostname/motion";
      $("#storage_template").value =
        storage.template || "%Y%m%d/%H/%Y%m%dT%H%M%S";
      $("#storage_send_photo").checked =
        storage.send_photo !== false && storage.send_photo !== "false";
      $("#storage_send_video").checked =
        storage.send_video === true || storage.send_video === "true";
    });
  }
  ...
  if (form) {
    form.addEventListener("submit", (event) =>
      send2Save("Storage", form, event, () => {
        const devicePath =
          $("#storage_device_path").value.trim() || "%hostname/motion";
        const template =
          $("#storage_template").value.trim() || "%Y%m%d/%H/%Y%m%dT%H%M%S";
        return {
          storage: {
