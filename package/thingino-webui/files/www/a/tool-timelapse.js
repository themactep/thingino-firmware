(function () {
  ...
  function applyTimelapseState(tl = {}) {
    ...
    if (timelapseKeepDays)
      timelapseKeepDays.value =
        typeof tl.keep_days === "number" ? tl.keep_days : tl.keep_days || 7;
    if (timelapsePreset)
      timelapsePreset.checked = boolFromValue(tl.preset_enabled);
  ...
  function renderDebug(debug = {}) {
    if (debugTimelapse)
      debugTimelapse.textContent =
        debug.timelapse || "No timelapse data available.";
    if (debugCrontab)
  ...
  function applyState(data = {}) {
    ...
    populateMountSelect(
      timelapseMount,
      mounts,
      data.timelapse && data.timelapse.mount,
    );
  ...
  function buildTimelapseParams() {
    ...
    params.set(
      "tl_preset_enabled",
      isChecked(timelapsePreset) ? "true" : "false",
    );
  ...
  if (timelapseMount)
    timelapseMount.addEventListener("change", updateTimelapseLink);
  if (timelapseFilepath)
    timelapseFilepath.addEventListener("input", updateTimelapseLink);
