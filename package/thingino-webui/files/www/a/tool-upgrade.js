(function () {
  ...
  async function loadState() {
    ...
    try {
      ...
      if (!response.ok || payload.error) {
        const message = getNested(
          payload,
          ["error", "message"],
          "Unable to load flash metadata.",
        );
  ...
  async function uploadFirmware() {
    ...
    try {
      ...
      showAlert(
        "success",
        payload.message || "Firmware uploaded. Ready to flash.",
      );
      startCommand(
        payload.command || getNested(metadata, ["upload", "command"], ""),
        { reboot: true },
      );
      firmwareInput.value = "";
