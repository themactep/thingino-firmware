(function () {
  const usageLabel = $("#overlayUsageLabel");
  ...
  function decodeListing(payload) {
    ...
    const encoded =
      typeof payload.listing_base64 === "string"
        ? payload.listing_base64.trim()
        : "";
    if (!encoded) {
    ...
    return (
      ...
      (typeof payload.listing === "string" ? payload.listing : "")
    );
  }
  ...
  async function loadOverlay() {
    ...
    showAlert();
    try {
      const response = await fetch("/x/info-overlay.cgi", {
