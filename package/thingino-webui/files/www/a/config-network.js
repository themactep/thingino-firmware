(function () {
  ...
  function toggleCardAvailability(card, enabled) {
    ...
    card
      .querySelectorAll(
        ".card-body input, .card-body button, .card-body select",
      )
      .forEach((el) => {
  ...
  function toggleDhcpFields(card, dhcpEnabled) {
    card.querySelectorAll("[data-static-fields] input").forEach((input) => {
      input.disabled =
        dhcpEnabled ||
        input.closest(".iface-card").classList.contains("iface-disabled");
    });
  }
  ...
  function applyInterfaceState(card, data) {
    card.querySelector(".iface-enabled").checked =
      data.enabled === true || data.enabled === "true";
    card.querySelector(".iface-dhcp").checked =
      data.dhcp === true || data.dhcp === "true";
    card.querySelector(".iface-ipv6").checked =
      data.ipv6 === true || data.ipv6 === "true";
  ...
  async function saveConfig(ev) {
    ...
    try {
      ...
      showAlert("", "");
      showOverlayMessage(
        "Network settings updated. A reboot may be required to apply everything.",
        "success",
      );
      await loadConfig();
    } catch (err) {
