(function () {
  ...
  function showFormatConfirmModal(fs) {
    ...
    if (window.bootstrap && window.bootstrap.Modal) {
      formatConfirmModalInstance =
        window.bootstrap.Modal.getOrCreateInstance(formatConfirmModalEl);
  ...
  function renderDeviceSummary(device = {}) {
    ...
    if (lastState && lastState.reports) {
      ...
      if (mountsText) {
        ...
        const firstMount = mountLines.find(
          (line) => line.trim() && line.includes("/dev/mmc"),
        );
        if (firstMount) {
  ...
  function getSelectedFs() {
    ...
    const selected = formatOptions.querySelector(
      'input[name="fstype"]:checked',
    );
  ...
  function populateFilesystems(filesystems = []) {
    ...
    filesystems.forEach((fs, index) => {
      ...
      wrapper.className =
        "form-check border rounded-3 px-3 py-2 position-relative";
      const input = document.createElement("input");
      input.type = "radio";
      input.className =
        "form-check-input position-absolute top-50 translate-middle-y";
      ...
      if (fs.recommended) {
        ...
        badge.className =
          "badge text-bg-success position-absolute top-0 end-0 mt-2 me-2";
  ...
  function applyState(data = {}) {
    ...
    partitionOutput.textContent =
      partitionsText || "No df entries found for /dev/mmc.";
  ...
  async function loadState() {
    ...
    try {
      ...
      dispatchAlert(
        "danger",
        err && err.message ? err.message : "Failed to query SD card state.",
      );
      const statusMessage =
        err && err.message
          ? `Unable to load SD card information: ${err.message}`
          : "Unable to load SD card information.";
  ...
  formatForm.addEventListener("submit", (ev) => {
    ...
    if (!lastState || !lastState.has_sdcard) {
      dispatchAlert(
        "warning",
        "Insert an SD card before attempting to format.",
      );
      return;
    }
  ...
  if (copyFormatLog) {
    copyFormatLog.addEventListener("click", () => {
      const rawLog =
        formatLog && formatLog.dataset && formatLog.dataset.rawLog
          ? formatLog.dataset.rawLog
          : formatLog.textContent;
      ...
      if (!clipboard || typeof clipboard.copy !== "function") {
        dispatchAlert(
          "warning",
          "Clipboard support is not available in this browser.",
        );
        return;
      }
      clipboard
        .copy(rawLog)
        .then(() => {
          copyFormatLog.innerHTML =
            '<i class="bi bi-clipboard-check me-1"></i>Copied';
          copyFormatLog.classList.replace(
