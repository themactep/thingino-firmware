(function () {
  ...
  const ACTIONS = {
    wipeoverlay: {
      ...
      description:
        "Erase all files stored in the overlay partition. Most customizations will be lost.",
    },
    fullreset: {
      title: "Reset firmware to defaults",
      description:
        "Factory reset the firmware and overlay partition. All settings and files will be removed.",
    },
  };
  ...
  if (!meta) {
    statusText.textContent =
      "Select an action from the Reset page to continue.";
    showAlert(
      "warning",
      "No reset action selected. Use the Reset page to choose what to wipe.",
    );
    return;
  }
  ...
  async function requestCommand() {
    statusText.innerHTML =
      '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Preparing reset command...';
    try {
      ...
      if (!response.ok || (payload && payload.error)) {
        const message =
          payload && payload.error && payload.error.message
            ...
            : "Failed to prepare reset command.";
      ...
      outputEl.dispatchEvent(
        new CustomEvent("thingino:start-command", { detail }),
      );
    } catch (err) {
      statusText.textContent = "Reset aborted.";
      showAlert(
        "danger",
        err.message || "Unable to start the requested reset action.",
      );
    }
  }
