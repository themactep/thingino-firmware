(function () {
  ...
  function formatUsageSummary(section) {
    ...
    const resolvedFree = Number.isFinite(free)
      ? Math.max(0, free)
    ...
    updateLegend("overlay-legend", [
      {
        color: "#b084f7",
        label: "Used",
        value: overlay.used || 0,
        total: overlay.total || 1,
      },
    ]);
  ...
  async function loadUsage(options = {}) {
    ...
    try {
      ...
      showSpinner(false);
      showError(
        err && err.message ? err.message : "Unable to load usage data.",
      );
    } finally {
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initUsagePage, {
      once: true,
    });
  } else {
    initUsagePage();
