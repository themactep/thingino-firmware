(function () {
  ...
  function populateInterfaces(list) {
    ...
    interfaceSelect.value =
      (metadata && metadata.defaults && metadata.defaults.interface) || "auto";
  }
  ...
  function populateActions(actions = []) {
    ...
    const defaultAction =
      (metadata && metadata.defaults && metadata.defaults.action) ||
      (actions[0] && actions[0].id) ||
      "ping";
  ...
  function updateActionHelp() {
    ...
    const description =
      actionDescriptions.get(actionId) || "Run a diagnostic test.";
  ...
  async function loadMetadata() {
    ...
    try {
      ...
      packetSizeInput.value =
        defaults.packet_size || packetSizeInput.value || packetSizeInput.min;
      ...
      updateStatus(
        "Metadata unavailable. Some features may not work.",
        "danger",
      );
      showAlert(
        "danger",
        err.message || "Failed to load metadata for this tool.",
      );
    }
  }
    ...
    try {
      ...
      if (!response.ok || (payload && payload.error)) {
        const message =
          payload && payload.error && payload.error.message
            ? payload.error.message
            : "Unable to start the requested test.";
        throw new Error(message);
      }
      ...
      outputEl.dispatchEvent(
        new CustomEvent("thingino:start-command", {
  ...
  function copyOutput() {
    ...
    clipboard
      ...
      .catch(() => {
        showAlert(
          "warning",
          "Unable to copy output to clipboard on this browser.",
        );
      });
  }
  ...
  wrapButton.addEventListener("click", () => {
    ...
    wrapButton.title = wrapping
      ? "Line wrap on \u2014 click to disable"
      : "Line wrap off \u2014 click to enable";
  });
