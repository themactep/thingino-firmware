...
do_save_event() {
	...
	[ -n "$sound" ] && jct "$TMP_FILE" set "chime.events.$event.sound" "$sound" >/dev/null 2>&1
	[ -n "$vol" ] && jct "$TMP_FILE" set "chime.events.$event.volume" "$vol" >/dev/null 2>&1
	[ -n "$rep" ] && jct "$TMP_FILE" set "chime.events.$event.repeat" "$rep" >/dev/null 2>&1
