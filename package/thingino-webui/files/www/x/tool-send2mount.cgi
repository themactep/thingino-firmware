...
if [ "POST" = "$REQUEST_METHOD" ]; then
	error=""
	read_from_post "send_mount_save" "dir template video"
	[ -z "$send_mount_save_dir" ] && error="Directory mount cannot be empty."
	defaults

	if [ -z "$error" ]; then
		save2config "
send_mount_save_dir=\"$send_mount_save_dir\"
...
<form action="<%= $SCRIPT_NAME %>" method="post" class="mb-4">
	<div class="row row-cols-1 row-cols-md-2">
		<div class="col">
			<% field_text "send_mount_save_dir" "Local directory mount" "Absolute path to save files" %>
