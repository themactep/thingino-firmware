#!/bin/sh
# shellcheck disable=SC1091,SC2046,SC2155,SC3043
...
verify_password() {
	...
	# Note: must use -m sha512 (no dash) for correct encryption
	local test_hash=$(mkpasswd -m sha512 "$password" "$salt" 2>/dev/null)
