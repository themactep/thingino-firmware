...
if [ "POST" = "$REQUEST_METHOD" ]; then
  ...
  # read posted values
  read_from_post "save" "dir template video"
...
<form action="<%= $SCRIPT_NAME %>" method="post" class="mb-4">
  <div class="row row-cols-1 row-cols-md-2">
    ...
    <div class="col">
      <% field_checkbox "save_video" "Save video buffer instead of snapshot" %>
    </div>
  </div>
  <% button_submit %>
</form>

<button type="button" class="btn btn-dark border mb-2" title="Save Now" data-sendto="local">Test</button>
