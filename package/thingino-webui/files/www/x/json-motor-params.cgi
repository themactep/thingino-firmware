...
steps_pan_val=$(jct /etc/thingino.json get motors.steps_pan 2>/dev/null)
[ -z "$steps_pan_val" ] && steps_pan_val=0
steps_tilt_val=$(jct /etc/thingino.json get motors.steps_tilt 2>/dev/null)
[ -z "$steps_tilt_val" ] && steps_tilt_val=0
accel_pan_val=$(jct /etc/thingino.json get motors.accel_pan 2>/dev/null)
[ -z "$accel_pan_val" ] && accel_pan_val=0
accel_tilt_val=$(jct /etc/thingino.json get motors.accel_tilt 2>/dev/null)
[ -z "$accel_tilt_val" ] && accel_tilt_val=0
motion_driver_val=$(jct /etc/thingino.json get motors.motion_driver 2>/dev/null)
[ -z "$motion_driver_val" ] && motion_driver_val=legacy
preview_control_mode_val=$(jct /etc/thingino.json get motors.preview_control_mode 2>/dev/null)
[ -z "$preview_control_mode_val" ] && preview_control_mode_val=step
pos_0_val=$(jct /etc/thingino.json get motors.pos_0 2>/dev/null)
