...
THINGINO_GPIO_CAMERA_CONF = $(BR2_EXTERNAL_THINGINO_PATH)/$(CAMERA_SUBDIR)/$(CAMERA)/gpio.conf
...
define INSTALL_GPIO_CAMERA_CONF
	if [ -f "$(THINGINO_GPIO_CAMERA_CONF)" ]; then \
		echo "" >> $(TARGET_DIR)/etc/gpio.conf; \
		cat "$(THINGINO_GPIO_CAMERA_CONF)" >> $(TARGET_DIR)/etc/gpio.conf; \
	fi
endef

THINGINO_GPIO_POST_INSTALL_TARGET_HOOKS += INSTALL_GPIO_CONF INSTALL_GPIO_CAMERA_CONF
