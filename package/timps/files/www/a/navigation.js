(function () {
  ...
  function buildDefaultMenu() {
    ...
    // GPIO pins, Doorbell Chime, Pan/Tilt motors, Telegram Bot, WireGuard VPN
    // and ZeroTier VPN are now injected by their own packages' *.webui.json
    // manifests via applyPluginNav() below (each package installs its own
    // page/CGI and only exists in the image when actually selected, so the
    // old hasMotors-style runtime toggle is no longer needed here). Dusk2Dawn
    // used to be the stock thingino-daynightd settings page; that daemon is
    // never built on timps images (timps does its own day/night detection,
    // see timps.mk's TIMPS_DISABLE_DAYNIGHTD), and its page moved into
    // thingino-daynightd's own package, so it no longer ships at all - the
    // link is dropped rather than left dangling. Photosensing stays: it's
    // timps's own native detection UI (config-photosensing.js talks straight
    // to /control), not the stock page, and no plugin covers it for us.
    const settingsItems = [
