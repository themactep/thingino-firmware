...
# NOTE: re-run the WebUI plugin assembler, LAST. This must stay the final entry
# in TIMPS_TARGET_FINALIZE_HOOKS.
#
# thingino-webui registers THINGINO_WEBUI_ASSEMBLE_PLUGINS as its own finalize
# hook. Buildroot appends per-package finalize hooks in package-evaluation
# order, and external.mk globs package/*/*.mk alphabetically, so
# "thingino-webui" is evaluated before "timps" and its assembly runs BEFORE
# every hook above. Two things break as a result on a timps image:
#
...
#     navigation.js applyPluginNav() splices nothing in - so the nav menu
#     silently loses every plugin-contributed entry (GPIO, Motors, Doorbell,
#     Telegram, WireGuard, ZeroTier) while the pages we do NOT override still
#     show them. That asymmetry is what makes the menu look intermittent.
#
#  2. TIMPS_DISABLE_DAYNIGHTD deletes thingino-daynightd's installed
#     daynightd.webui.json, but only after plugins.js was already generated
#     from it - so the generated plugins.js still carries a "daynightd" key,
#     re-adding a duplicate "Photosensing" entry next to timps's native one
#     plus a "Dusk2Dawn" link to a page this image does not ship.
#
# Re-running the assembler here fixes both: it regenerates plugins.js from the
# manifests that actually survived, and injects the tags into timps's pages.
# The injectors are guarded by has_script_src(), so the second pass is a no-op
# on pages the first pass already handled and nothing is double-injected.
ifeq ($(BR2_PACKAGE_THINGINO_WEBUI),y)
define TIMPS_REASSEMBLE_WEBUI_PLUGINS
	@python3 "$(THINGINO_WEBUI_PKGDIR)/scripts/assemble_plugins.py" "$(TARGET_DIR)" || \
		printf 'timps: plugin re-assembly failed (non-fatal)\n'
	# assemble_plugins.py bails out early ("no plugin manifests found") without
	# touching plugins.js when nothing is left to assemble. On an image whose
	# only manifest was daynightd's - which the hook above just deleted - that
	# would leave the previously generated plugins.js in place, still naming a
	# plugin this image does not ship. Overwrite it with an empty stub instead.
	if [ -f $(TARGET_DIR)/var/www/a/plugins.js ] && \
	   ! ls $(TARGET_DIR)/var/www/a/plugins/*.webui.json >/dev/null 2>&1 ; then \
		printf '%s\n' \
			'// Generated by timps: no webui plugin manifests in this image' \
			'(function(){' \
			'  "use strict";' \
			...
			'})();' \
			> $(TARGET_DIR)/var/www/a/plugins.js ; \
	fi
endef
TIMPS_TARGET_FINALIZE_HOOKS += TIMPS_REASSEMBLE_WEBUI_PLUGINS
endif
