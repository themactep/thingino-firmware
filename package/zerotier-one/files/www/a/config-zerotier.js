...
async function removeNetwork(_nwid) {
  const confirmed = await confirm(
    "Remove this network? This will disconnect and delete the configuration.",
  );
  if (!confirmed) {
    return;
  }
...
async function toggleAutostart(nwid, on_boot) {
  try {
    ...
    showOverlayMessage(
      `Auto-start ${on_boot ? "enabled" : "disabled"}`,
      "success",
    );
    await loadData();
  } catch (error) {
...
function renderNetworks() {
  ...
  for (const [nwid, config] of Object.entries(networks)) {
    ...
    const isConnected =
      ztStatus.running && ztStatus.online && ztStatus.networkStatus;
    ...
    const displayTitle =
      liveNetworkName && liveNetworkName !== nwid
    ...
    if (isConnecting) {
      ...
      // Or if we're waiting for authorization (network joined but not authorized)
        ...
        ztStatus.networkStatus === "ACCESS_DENIED" ||
        ztStatus.networkStatus === "REQUESTING_CONFIGURATION"
      ) {
        isConnecting = false;
      }
      // Or if timed out
        ...
        connectionStartTime &&
        Date.now() - connectionStartTime > 30000
      ) {
        isConnecting = false;
      }
    }
    ...
    // Check for timeout (30 seconds)
    const hasTimedOut =
      connectionStartTime && Date.now() - connectionStartTime > 30000;
    ...
    if (isConnected && networkStatus === "OK") {
      ...
      connectionStartTime = null;
      statusText =
        'Network may not exist or be unreachable. <a href="javascript:void(0)" class="retry-connection" data-nwid="' +
        nwid +
        '">Retry</a>';
    } else if (isConnected) {
      ...
      <button type="button" class="btn ${isConnected ? "btn-warning" : "btn-primary"} network-toggle-btn" data-nwid="${nwid}" ${isConnecting || isDisconnecting ? "disabled" : ""} style="min-width: 145px;">
        <span class="button-text">${
          isDisconnecting
            ? "Disconnecting..."
            : isConnecting
              ? ztStatus.running && ztStatus.online
                ? "Joining network..."
                : ztStatus.running
                  ? "Initializing..."
                  : "Connecting..."
