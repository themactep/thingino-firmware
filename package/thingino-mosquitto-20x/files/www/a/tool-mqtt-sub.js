(function () {
  ...
  const PRESETS = [
    ...
    {
      label: "Log to syslog",
      cmd: 'logger -t mqtt "Topic: $MQTT_TOPIC Payload: $MQTT_PAYLOAD"',
    },
    {
      label: "Conditional reboot (pay=1)",
      cmd: '[ "$MQTT_PAYLOAD" = "1" ] && reboot',
    },
    {
      label: "Telegram hub agent",
      cmd: 'telegram-cam-agent "$MQTT_PAYLOAD"',
  ...
  function addSubscriptionRow(data) {
    ...
    inputAction.className =
      "form-control form-control-sm sub-action font-monospace";
  ...
  function setFormBusy(state) {
    [saveBtn, reloadBtn, addBtn].forEach(function (el) {
  ...
  async function loadConfig(opts) {
    ...
    try {
      ...
      showAlert(
        "danger",
        err.message || "Unable to load MQTT subscription settings.",
      );
    } finally {
  ...
  async function saveConfig(payload) {
    ...
    try {
      ...
      if (!resp.ok || (result && result.error)) {
        throw new Error(
          (result.error && result.error.message) || "Failed to save settings",
        );
      }
      if (
        ...
        window.thinginoFooter.showMessage(
          "MQTT subscription settings saved.",
          "success",
        );
      } else {
      ...
      showAlert(
        "danger",
        err.message || "Failed to save MQTT subscription settings.",
      );
    } finally {
      setFormBusy(false);
    }
  ...
  if (restartBtn) {
    restartBtn.addEventListener("click", async function () {
      try {
        ...
        showAlert(
          "danger",
          err.message || "Failed to restart MQTT subscription service.",
        );
      } finally {
        hideBusy();
