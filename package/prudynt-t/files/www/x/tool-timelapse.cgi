...
# invokes the `timelapse` snapshot command. Streamer-specific; the shared
# tool-record.cgi keeps the video recorder domain only.
...
build_state_payload() {
	...
	local mounts_json_str debug_video debug_timelapse debug_crontab tl_interval_num tl_keep_days_num
	...
	cat <<EOF
{
  ...
  "debug": {
    "timelapse": "$(json_escape "$debug_timelapse")",
    "crontab": "$(json_escape "$debug_crontab")"
  }
}
EOF
}
