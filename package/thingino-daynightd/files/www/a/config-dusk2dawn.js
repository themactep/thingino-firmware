  ...
  async function saveConfig() {
    ...
    sunParams.forEach((param) => {
      ...
      if (el.type === "checkbox") {
        ...
        sun[param] =
          param.endsWith("_offset") && !Number.isNaN(numeric)
            ? numeric
            : el.value || "";
      }
    });
    ...
    const tryIPGeo = async () => {
      try {
        ...
        applyCoords(
          data.latitude,
          data.longitude,
          "Location set from IP address (city-level accuracy). Adjust if needed.",
        );
      } catch (e) {
        setReady();
        showAlert(
          "warning",
          'Could not determine location automatically. Use the "Find manually" link.',
        );
      }
    };
