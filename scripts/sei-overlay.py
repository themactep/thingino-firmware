...
def generate_srt(groups, timeline):
    ...
    # Build per‑second interpolation schedule
    # For each second in the video, compute interpolated time text
    total_end = timeline[-1]["pts"] + 0.1

    # Build lookup: which group covers a given PTS
    def find_group(t):
        for g in groups:
            if g["start"] <= t < g["end"]:
                return g
    ...
    # Generate time-text events at 1‑second intervals
    time_events = []  # (start_pts, end_pts, text, sei_element)
    t = 0.0
    while t < total_end:
        sub_end = min(t + 1.0, total_end)
        group = find_group(t + 0.001)
    ...
    ass += "[V4+ Styles]\n"
    ass += "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, "
    ass += "OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, "
    ass += "ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, "
    ass += "Alignment, MarginL, MarginR, MarginV, Encoding\n"
    ass += f"Style: Default,{font_name},32,&H00FFFFFF,&H00000000,"
    ass += f"&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,{border_width},0,"
    ass += "2,10,10,10,1\n\n"
    ass += "[Events]\n"
    ass += "Format: Layer, Start, End, Style, Name, MarginL, MarginR, "
    ass += "MarginV, Effect, Text\n"
    ...
    for start_pts, end_pts, text, elem in time_events:
        ...
        alignment = _ass_alignment(x_raw, y_raw)
        ml = _ass_margin(x_raw, 384)
        mr = _ass_margin(-x_raw if x_raw < 0 else 0, 384) if x_raw < 0 else 0
        mv = _ass_margin(y_raw, 288) if y_raw > 0 else (_ass_margin(-y_raw, 288) if y_raw < 0 else 0)

        # For ASS alignment, margins work differently:
        # \an7 (top-left): MarginL from left, MarginV from top
        # \an9 (top-right): MarginR from right, MarginV from top
        # \an8 (top-center): MarginV from top, L/R ignored
        # We set MarginL and MarginR both, but only the relevant one is used
        # based on alignment.
        ...
        outline = _ass_color(stroke_hex)

        # Build the ASS dialogue line with inline styling overrides
        text_escaped = text.replace("\\", "\\\\").replace("\n", "\\N")
        tag = (
            f"{{\\an{alignment}"
            f"\\fs{fs}"
        ...
        ass += (
            f"Dialogue: 0,{start_ass},{end_ass},Default,,"
            f"{margin_l:04d},{margin_r:04d},{margin_v:04d},,"
            f"{tag}{text_escaped}\n"
        )
