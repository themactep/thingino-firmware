...
if ! git -C "$buildroot_path" diff --quiet; then
	echo "Stashing local changes in buildroot submodule..."
	git -C "$buildroot_path" stash push -m "buildroot update $(date +%Y-%m-%d)"
fi
