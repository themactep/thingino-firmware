...
Thingino installation), the radio already knows about it.  Run `pair`
**without** `-D` to register it in `thingino.json` without disturbing
the existing radio pairing:

```
doorbell_ctrl pair living_room
```

The full pairing sequence runs, but since the chime is already in the
